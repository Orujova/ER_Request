...
const formDataSlice = createSlice({
  ...
  reducers: {
    ...
    setActiveTab: (state, action) => {
      ...
      // Only reset if actually changing tabs
      if (newTab !== oldTab) {
        console.log(`Switching from ${oldTab} tab to ${newTab} tab`);

        // Reset form fields based on which tab we're switching to
        if (newTab === "employee") {
          // When switching to employee tab, reset employee-specific fields
          state.selectedEmployee = null;
          state.projectId = null;

          // Reset search queries for employee tab
          state.searchQueries.badge = "";
          state.searchQueries.name = "";
        } else if (newTab === "general") {
          // When switching to general tab, reset general-specific fields
          state.projectId = null;

          // Reset search query for project
          state.searchQueries.project = "";
        }

        // Common fields to reset regardless of which tab we're switching to
        state.requestType = "";
        state.subCase = "";
        state.ccAddresses = "";
        state.mailBody = "";
        state.selectedCCUsers = [];
        state.hyperlinks = [""];
        state.actFiles = [];
        state.presentationFiles = [];
        state.explanationFiles = [];
        state.otherFiles = [];

        // Reset search queries for common fields
        state.searchQueries.case = "";
        state.searchQueries.subCase = "";
      }
